...
bool SoundClass::Initialize(HWND hwnd,char* bgmfile, char* SEfile, char* Wfile, char* Lfile)
{
	bool result;
	...
	result = LoadWaveFile(SEfile, &m_secondaryBuffer2);
	if (!result)
	{
		return false;
	}

	result = LoadWaveFile(Wfile, &m_secondaryBuffer3);
	if (!result)
	{
		return false;
	}

	result = LoadWaveFile(Lfile, &m_secondaryBuffer4);
	if (!result)
	{
		return false;
	}
