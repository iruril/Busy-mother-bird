...
{
	bool result;
	m_collider = new ColliderType;//콜라이더 변수 동적할당 추가
...
void ModelClass::SetCollider(D3DXMATRIX scale, D3DXMATRIX trans)//콜라이더 값 설정 추가
{
	m_collider->Mx = trans.m[3][0] + ((Max - Mix) * scale.m[0][0]) / 2.0f;
	m_collider->mx = trans.m[3][0] - ((Max - Mix) * scale.m[0][0]) / 2.0f;
	m_collider->My = trans.m[3][1] + ((Max - Mix) * scale.m[1][1]) / 2.0f;
	m_collider->my = trans.m[3][1] - ((Max - Mix) * scale.m[1][1]) / 2.0f;
	m_collider->Mz = trans.m[3][2] + ((Max - Mix) * scale.m[2][2]) / 2.0f;
	m_collider->mz = trans.m[3][2] - ((Max - Mix) * scale.m[2][2]) / 2.0f;
}

void ModelClass::SetCollider(float Xpos, float Ypos, float Zpos)//콜라이더 설정 추가 (지름이 5인 고정형태의 콜라이더 생성. 플레이어용)
{
	m_collider->Mx = Xpos + 5 / 2;
	m_collider->mx = Xpos - 5 / 2;
	m_collider->My = Ypos + 5 / 2;
	m_collider->my = Ypos - 5 / 2;
	m_collider->Mz = Zpos + 5 / 2;
	m_collider->mz = Zpos - 5 / 2;
}

bool ModelClass::checkCollider(float max, float mix, float may, float miy, float maz, float miz)//충돌 체크 함수 추가, 충돌 시 트루, 그 외 펄스
{
